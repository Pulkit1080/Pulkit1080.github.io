...
const EXPERIENCE = [
  {
    ...
    bullets: [
      "Cut catalogue processing time by 35% with Python automation on ERP Next.",
      "Surfaced 64% zero-sales SKUs from 2 years of sales data using ABC analysis.",
      "Built a category-mapping system across 100+ products."
    ]
  },
  {
    when: "2025 - 2026",
    ...
    bullets: [
      "1st place, Dean's Cup Round 1 · Top-3 every round.",
  ...
  },
  {
    ...
    tags: ["Tableau", "QlikView", "Qlik Sense", "SQL", "AWS Athena", "Python", "Excel", "Agile"],
    bullets: [
...
const PROJECTS = [
  {
    ...
    problem: "Four cases across the Dean's Cup: Agentic AI in financial services (R1), Canadian banking M&A (R2), AI-enabled lending (R3), and consulting strategy (Finale, the only live round).",
    approach: "Each case: 4-person team, senior industry judges, compressed sprint. Won R1; top-3 every round.",
    metrics: [
      ["1st", "Round 1"],
      ["Top-3", "every round"],
      ["4", "rounds"],
    ...
    links: [
      ...
      { label: "R2 Deck · oneRBC", href: "assets/DeansCup_R2_KeepinItCaseual_Deck.pptx" },
      { label: "R3 Deck · Blanc Labs", href: "assets/DeansCup_R3_KeepinItCaseual_Deck.pptx" },
      { label: "Finale Deck · Optimus", href: "assets/DeansCup_Finale_KeepinItCaseual_Deck.pptx" }
    ]
  },
  {
    cat: "analytics",
    ...
    contain: false,
    problem: "Commercial leaders had no single pipeline-to-revenue view across geographies. KPIs lived in Excel and decks.",
    approach: "Scoped requirements with 4 stakeholder groups, designed the semantic layer, shipped 4 Tableau dashboards, ran hypercare.",
    metrics: [
      ["4", "dashboards"],
      ["12+", "stakeholders"],
      ["Tableau", "+ Qlik Sense"],
  ...
  },
  {
    ...
    contain: false,
    problem: "Validation across a sprawling QlikView stack: slow loads, expensive ad-hoc SQL, defects piling up against tight SLAs.",
    approach: "Built intermediate tables in S3 + AWS Athena to pre-stage validation queries. Repeatable test scaffolding instead of one-off SQL. Restructured data models, codified review standards, mentored juniors.",
    metrics: [
      ["~50%", "faster testing"],
      ["1,000+", "enhancements"],
      ["15", "exec dashboards"],
  ...
  },
  {
    ...
    contain: false,
    problem: "Manual image upload, validation, and category tagging across 100+ SKUs was the bottleneck.",
    approach: "Python scripts for ERP Next image handoff + validation. Category-mapping taxonomy. Zero-sales SKU analysis.",
    metrics: [
      ["-35%", "ops time"],
      ["64%", "zero-sales SKUs"],
      ["100+", "products"],
...
const WINS = [
  { rank: "1", sup: "st", where: "Dean's Cup · Round 1", detail: "Schulich · 2025–26", img: "assets/deans-cup-r1-team.jpg" },
  { rank: "1", sup: "st", where: "Ivey Soccer Tournament", detail: "Captain · Schulich, 2025", img: "assets/soccer-team.jpg" },
  { rank: "3", sup: "rd", where: "Unite India Gamethon", detail: "3rd / 450 nationally · 2019", img: "assets/unite-india-gamethon.png" }
];
...
if (projList) {
  ...
  function updateCount() {
    const visible = projList.querySelectorAll('.proj-card:not([hidden])').length;
    const total = PROJECTS.length;
    document.getElementById('filter-count').textContent = `${visible} of ${total}`;
  }
  updateCount();
...
if (lightbox && lightboxImg) {
  document.querySelectorAll('.portrait img, .beyond-card .img img, .community-photo img, .win-thumb img, .proj-visual img, .proj-gallery img').forEach(img => {
